...
class TestVortexSqlIntegration:
    ...
    def test_vortex_file_operations_with_catalog_table(
        ...
        read_data = read_vortex_file(f"file://{vortex_file_path}", table.io)
    ...
    def test_catalog_persistence_vortex_tables(
        self, tmp_path: Path, simple_schema: Schema, simple_arrow_data: pa.Table
    ) -> None:
...
def test_vortex_sql_catalog_end_to_end(tmp_path: Path) -> None:
    ...
    try:
        ...
        # Read the Vortex file back
        vortex_data = read_vortex_file(f"file://{vortex_file_path}", table.io)
