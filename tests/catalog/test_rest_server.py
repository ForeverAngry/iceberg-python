...
def test_partition_spec() -> PartitionSpec:
    ...
    return PartitionSpec(
        spec_id=0, fields=[PartitionField(source_id=3, field_id=1000, transform=IdentityTransform(), name="category")]
    )
...
def test_create_namespace(rest_server_app: TestClient) -> None:
    ...
    namespace_data = {"namespace": ["test_db"], "properties": {"owner": "test_user", "description": "Test database"}}
...
def test_create_duplicate_namespace(rest_server_app: TestClient) -> None:
    ...
    namespace_data = {"namespace": ["duplicate_test"], "properties": {"owner": "user1"}}
...
def test_list_namespaces_with_data(rest_server_app: TestClient) -> None:
    ...
    # Create a namespace
    namespace_data = {"namespace": ["list_test_db"], "properties": {"owner": "test_user"}}
...
def test_load_namespace(rest_server_app: TestClient) -> None:
    ...
    namespace_data = {"namespace": ["load_test_db"], "properties": {"owner": "test_user", "created_at": "2025-10-15"}}
    rest_server_app.post("/v1/namespaces", json=namespace_data)
...
def test_update_namespace_properties(rest_server_app: TestClient) -> None:
    ...
    # Create namespace
    namespace_data = {"namespace": ["update_test_db"], "properties": {"owner": "original_user"}}
    rest_server_app.post("/v1/namespaces", json=namespace_data)

    # Update properties
    update_data = {"updates": {"owner": "new_user", "env": "production"}}
    response = rest_server_app.post("/v1/namespaces/update_test_db/properties", json=update_data)
    assert response.status_code == 200
...
def test_update_namespace_properties_remove(rest_server_app: TestClient) -> None:
    ...
    namespace_data = {"namespace": ["remove_prop_test"], "properties": {"owner": "user", "temp_property": "to_be_removed"}}
    rest_server_app.post("/v1/namespaces", json=namespace_data)

    # Remove property
    update_data = {"removals": ["temp_property"]}
    response = rest_server_app.post("/v1/namespaces/remove_prop_test/properties", json=update_data)
    assert response.status_code == 200
...
def test_drop_namespace(rest_server_app: TestClient) -> None:
    ...
    # Create namespace
    namespace_data = {"namespace": ["drop_test_db"], "properties": {"owner": "test_user"}}
    rest_server_app.post("/v1/namespaces", json=namespace_data)
...
def test_list_tables_empty(rest_server_app: TestClient) -> None:
    ...
    # Create namespace
    namespace_data = {"namespace": ["empty_table_ns"], "properties": {"owner": "test"}}
    rest_server_app.post("/v1/namespaces", json=namespace_data)
...
def test_table_exists_check(rest_server_app: TestClient) -> None:
    ...
    # Create namespace
    namespace_data = {"namespace": ["exists_check_ns"], "properties": {"owner": "test"}}
    rest_server_app.post("/v1/namespaces", json=namespace_data)
...
def test_load_nonexistent_table(rest_server_app: TestClient) -> None:
    ...
    # Create namespace
    namespace_data = {"namespace": ["load_table_ns"], "properties": {"owner": "test"}}
    rest_server_app.post("/v1/namespaces", json=namespace_data)
...
def test_drop_nonexistent_table(rest_server_app: TestClient) -> None:
    ...
    # Create namespace
    namespace_data = {"namespace": ["drop_table_ns"], "properties": {"owner": "test"}}
    rest_server_app.post("/v1/namespaces", json=namespace_data)
...
def test_multi_level_namespace(rest_server_app: TestClient) -> None:
    ...
    namespace_data = {"namespace": ["org", "department", "team"], "properties": {"owner": "team_lead"}}
...
def test_list_namespaces_with_parent(rest_server_app: TestClient) -> None:
    ...
    parent_data = {"namespace": ["parent"], "properties": {"type": "parent"}}
    rest_server_app.post("/v1/namespaces", json=parent_data)

    # Create child namespace
    child_data = {"namespace": ["parent", "child"], "properties": {"type": "child"}}
...
def test_invalid_json(rest_server_app: TestClient) -> None:
    ...
    response = rest_server_app.post("/v1/namespaces", data="not valid json", headers={"Content-Type": "application/json"})
...
def test_complete_workflow(rest_server_app: TestClient) -> None:
    ...
    namespace_data = {"namespace": ["workflow_test"], "properties": {"owner": "data_team", "env": "test"}}
    response = rest_server_app.post("/v1/namespaces", json=namespace_data)
    assert response.status_code == 200
    ...
    update_data = {"updates": {"version": "1.0", "updated_by": "test_user"}}
    response = rest_server_app.post("/v1/namespaces/workflow_test/properties", json=update_data)
    assert response.status_code == 200
...
def test_multiple_namespaces_isolation(rest_server_app: TestClient) -> None:
    ...
    for ns in namespaces:
        namespace_data = {"namespace": [ns], "properties": {"owner": f"{ns}_owner"}}
        response = rest_server_app.post("/v1/namespaces", json=namespace_data)
        assert response.status_code == 200
    ...
    response = rest_server_app.post("/v1/namespaces/ns1/properties", json=update_data)
    assert response.status_code == 200
...
def test_create_many_namespaces(rest_server_app: TestClient) -> None:
    ...
    for i in range(num_namespaces):
        namespace_data = {"namespace": [f"perf_test_{i}"], "properties": {"index": str(i)}}
        response = rest_server_app.post("/v1/namespaces", json=namespace_data)
        assert response.status_code == 200
